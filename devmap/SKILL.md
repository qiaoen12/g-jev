...
- **开工总地图**：标题后先给“总任务、已完成、预计文件、当前阻塞”四个核心数字；中间紧接待开始、开发中、测试中、已完成四列看板。看板中只显示 Task 名称，阻塞时才加简短标记。一个 Task 占独立一行，同状态 Task 纵向排列；禁止用 `<br>`、字面量 `\n` 或其他 HTML 换行合并到同一单元格。空列仅首行显示“—”，后续行留空；长名称允许自然换行。不要把预期结果、完成条件或依赖做成主视图。
